...
// Scans and loads commands from src/features/**/commands, exposes a Map on client,
// and (optionally) registers slash commands via REST if the dependency is present.
//
// Safe for both CJS and ESM-shaped command modules.

const path = require('path');
const fs = require('fs').promises;

// Shared modules (new locations), wrapped in try/catch to avoid hard crashes
...
class CommandManager {
  constructor(client) {
    ...
    try {
      if (RateLimiterMod) {
        // many custom RateLimiter utilities accept { windowMs, max }
    ...
    // Where to scan for commands in the new structure
    this.scanBases = [ path.join(process.cwd(), 'src', 'features') ];
  }
  ...
  async getCommandFiles() {
    ...
    const walk = async (dir) => {
      ...
      for (const it of items) {
        ...
        if (it.isDirectory()) {
          await walk(full);
        } else if (it.isFile() && it.name.endsWith('.js') && full.includes(`${path.sep}commands${path.sep}`)) {
          results.push(full);
        }
      }
    };
  ...
  deriveNameFromFilename(filePath) {
    ...
    // support dash-separated file names, e.g. pvp-raid-history.js -> pvp-raid-history
    return base;
  }

  // Optional: register slash commands with Discord REST.
  // This will run if the REST dependency is available; otherwise it logs a hint and skips.
  async registerSlashCommands() {
    ...
    // Try discord.js first (v14 may not expose REST), then @discordjs/rest fallback
